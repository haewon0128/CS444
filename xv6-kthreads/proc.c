...
kthread_create(void (*func)(void*), void *arg_ptr, void *tstack)
{
	...
	cprintf("*** thread stack not page alligned ***\n");
        return -1;
    }
...
kthread_join(int tid)
{
	...
	for(p = ptable.proc; p<&ptable.proc[NPROC] ; p++)
	{
		if(p->state == UNUSED || p->parent == NULL || p->isThread == 0)
		{
			continue;
		}
...
sys_cps(void)
{
    ...
    for (i = 0; i < NPROC; i++) {
        if (ptable.proc[i].state != UNUSED) {
		   ...
		   ptable.proc[i].oncpu >= 0 ? cprintf("\t%d", ptable.proc[i].oncpu) : cprintf("\t");
	cprintf("\t%u\t%u\t%u"
