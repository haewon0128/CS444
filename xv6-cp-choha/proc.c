	...
	for(;;){
		...
		if(counter >0){
			...
			for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
				...
				switchkvm();
#ifdef PROC_TIME
				p->ticks_total = p->ticks_total + uptime() - p->ticks_begin;
...
sys_cps(void)
{
	...
	for (i = 0; i < NPROC; i++) {
		if (ptable.proc[i].state != UNUSED) {
			...
			cprintf("%d\t%d\t%s\t%s\t%u\t%u-%s%u-%u %u:%u:%u\t%u\t%u\t%d"
					, ptable.proc[i].pid
...
int renice(int nice, int pid)
{
	...
	struct proc *p;
	if(nice > MAX_NICE_VALUE || nice < MIN_NICE_VALUE)
	{
		return 1;
	}
